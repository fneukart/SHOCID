...
<%@ page import="shocid.cortical.CorticalJSP"%>
...
<body>
	<h1 class="heading">Cortical results</h1>
...
// 					+ request.getParameter("output");
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = CorticalJSP
					.getNumberInputNeurons();
        int numberOutputNeurons = Integer
        .valueOf(numberOutputNeuronsString).intValue();
		MLDataSet trainingSet = CorticalJSP
					.getTrainingSet();
			BasicNetwork network = CorticalJSP.getNetwork();
			int col = 0;
			double[][] inputValuesArray = CorticalJSP
					.getInputValues();
			double[][] outputValuesArray = CorticalJSP
					.getOutputValues();
			double[][] idealValuesArray = CorticalJSP
					.getIdealValues();
			double[] error = CorticalJSP.getError();
			int epochs = CorticalJSP.getEpochs();
		%>
		<div class="AccordionPanel">
			<div class="AccordionPanelTab">Error over generations - graph
				of cortical agent</div>
