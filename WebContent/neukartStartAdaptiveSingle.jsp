...
<%@ page import="shocid.neukart.adaptive.NeukartAdaptiveJSP"%>
...
<title>Adaptive Genetic Neukart Data Processing</title>
<link href="SpryAssets/SpryAccordion.css" rel="stylesheet"
	type="text/css">
</head>
<body>
	<h1 class="heading">Adaptive genetic Neukart 
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = NeukartAdaptiveJSP.getNumberInputNeurons();
			int numberOutputNeurons = Integer
					.valueOf(numberOutputNeuronsString).intValue();
			MLDataSet trainingSet = NeukartAdaptiveJSP.getTrainingSet();
			BasicNetwork network = NeukartAdaptiveJSP.getBestNetwork();
			int col = 0;
			double[][] inputValuesArray = NeukartAdaptiveJSP.getInputValues(0);
			double[][] outputValuesArray = NeukartAdaptiveJSP
					.getOutputValuesBestOverall();
			double[][] idealValuesArray = NeukartAdaptiveJSP.getIdealValues(0);
			double[] error = NeukartAdaptiveJSP.getErrorBestNetwork();
			double[] errorSelected;
			int epochs = NeukartAdaptiveJSP.getBestEpochs();
		%>
		<div class="AccordionPanel">
			<div class="AccordionPanelTab">Error over iterations - graph of
				genetic algorithm agent</div>
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				<%
					...
					if (NeukartAdaptiveJSP.getBestSHLNetworkQuality() < NeukartAdaptiveJSP
							.getBestMHLNetworkQuality()) {
						bestNetwork = NeukartAdaptiveJSP
				...
				<p>
					<%
						if (NeukartAdaptiveJSP.getBestSHLNetworkQuality() < NeukartAdaptiveJSP
								.getBestMHLNetworkQuality()) {
							hn1 = NeukartAdaptiveJSP.getBestHiddenLayer1Neurons();
					%>
					The network has
					<%=hn1%>
					...
					<%
							...
							hn1 = NeukartAdaptiveJSP.getBestHiddenLayer1Neurons();
							hn2 = NeukartAdaptiveJSP.getBestHiddenLayer2Neurons();
					%>
					The network has
					<%=hn1%>
					...
					<%
						...
						bestEpochs = NeukartAdaptiveJSP.getBestEpochs();
					%>
				...
				<table border=0 width=100%>
					<%
						...
						int epochsSHL = 0;
						for (int i = 0; i < NeukartAdaptiveJSP.getQualitySHLArray().length; i++) {
							qualitySHL = NeukartAdaptiveJSP.getQualitySHLArray(i);
							epochsSHL = NeukartAdaptiveJSP.getEpochsSL(i);
					%>
					<tr>
				...
				<table border=0 width=100%>
					<%
						...
						int epochsMHL = 0;
						for (int i = 0; i < NeukartAdaptiveJSP.getQualityMHLArray().length; i++) {
							qualityMHL = NeukartAdaptiveJSP.getQualityMHLArray(i);
							epochsMHL = NeukartAdaptiveJSP.getEpochsML(i);
					%>
					<tr>
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				<form id="form1" name="form1" method="post"
					action="neukartAdaptiveSingleDetails.jsp">
					<table>
						<tr>
							<td>
								<div id="SHLAgentDetails">
									<label for="SHLAgentDetails">SHL agent evolution
										...
										name="showSHLAgentDetails" size="1" id="showSHLAgentDetails">
											<%
												for (int networks = 0; networks < NeukartAdaptiveJSP
