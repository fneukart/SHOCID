...
<%@ page import="shocid.elman.ElmanJSP"%>
...
<body>
	<h1 class="heading">Elman prediction results</h1>
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = ElmanJSP
					.getNumberInputNeurons();
        int numberOutputNeurons = Integer
        .valueOf(numberOutputNeuronsString).intValue();
			MLDataSet trainingSet = ElmanJSP
					.getTrainingSet();
			BasicNetwork network = ElmanJSP.getNetwork();
			int col = 0;
			double[][] inputValuesArray = ElmanJSP
					.getInputValues();
			double[][] outputValuesArray = ElmanJSP
					.getOutputValues();
			double[][] idealValuesArray = ElmanJSP
					.getIdealValues();
			double[] error = ElmanJSP.getError();
			int epochs = ElmanJSP.getEpochs();
		%>
