...
<%@ page import="shocid.transgenetic.hostgeneticinformation.FFANNAISTransGeneticAlgorithmJSP"%>
...
<title>Transgenetic AIS Data Processing</title>
<link href="SpryAssets/SpryAccordion.css" rel="stylesheet"
	type="text/css">
</head>
<body>
	<h1 class="heading">Transgenetic AIS classification results</h1>
...
//  					+ request.getParameter("output");
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = FFANNAISTransGeneticAlgorithmJSP
					.getNumberInputNeurons();
        int numberOutputNeurons = Integer
        .valueOf(numberOutputNeuronsString).intValue();
			MLDataSet trainingSet = FFANNAISTransGeneticAlgorithmJSP
					.getTrainingSet();
			BasicNetwork network = FFANNAISTransGeneticAlgorithmJSP.getTGNetwork();
			int col = 0;
			double[][] inputValuesArray = FFANNAISTransGeneticAlgorithmJSP
					.getInputValues();
			double[][] outputValuesArray = FFANNAISTransGeneticAlgorithmJSP
					.getOutputValues();
			double[][] idealValuesArray = FFANNAISTransGeneticAlgorithmJSP
					.getIdealValues();
			double[] error = FFANNAISTransGeneticAlgorithmJSP.getTGError();
			int epochs = FFANNAISTransGeneticAlgorithmJSP.getTGEpochs();
		%>
				...
				<%
					...
					for (l = 0; l < arrData.length; l++) {
						...
						strXML = strXML + "<set name='" + arrData[l][0] + "' value='"
								+ arrData[l][1] + "' color='" + colorHelper.getFCColor() + "'/>";
