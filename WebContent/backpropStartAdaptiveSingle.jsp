...
<title>Adaptive Back Propagation Data Processing</title>
<link href="SpryAssets/SpryAccordion.css" rel="stylesheet"
	type="text/css">
</head>
<body>
	<h1 class="heading">Adaptive back propagation classification
		results</h1>
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = FFANNAdaptiveBackPropagationJSP
					.getNumberInputNeurons();
			int numberOutputNeurons = Integer
					.valueOf(numberOutputNeuronsString).intValue();
			MLDataSet trainingSet = FFANNAdaptiveBackPropagationJSP
					.getTrainingSet();
			BasicNetwork network = FFANNAdaptiveBackPropagationJSP
					.getBestNetwork();
			int col = 0;
			double[][] inputValuesArray = FFANNAdaptiveBackPropagationJSP
					.getInputValues(0);
			double[][] outputValuesArray = FFANNAdaptiveBackPropagationJSP
					.getOutputValuesBestOverall();
			double[][] idealValuesArray = FFANNAdaptiveBackPropagationJSP
					.getIdealValues(0);
			double[] error = FFANNAdaptiveBackPropagationJSP
					.getErrorBestNetwork();
			double[] errorSelected;
			int epochs = FFANNAdaptiveBackPropagationJSP.getBestEpochs();
		%>
		<div class="AccordionPanel">
			<div class="AccordionPanelTab">Error over iterations - graph of
				backpropagation algorithm agent</div>
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				<%
					...
					if (FFANNAdaptiveBackPropagationJSP.getBestSHLNetworkQuality() < FFANNAdaptiveBackPropagationJSP
							.getBestMHLNetworkQuality()) {
						bestNetwork = FFANNAdaptiveBackPropagationJSP
				...
				<p>
					<%
						if (FFANNAdaptiveBackPropagationJSP.getBestSHLNetworkQuality() < FFANNAdaptiveBackPropagationJSP
								.getBestMHLNetworkQuality()) {
							hn1 = FFANNAdaptiveBackPropagationJSP
					...
					<%
							...
							hn1 = FFANNAdaptiveBackPropagationJSP
									.getBestHiddenLayer1Neurons();
							hn2 = FFANNAdaptiveBackPropagationJSP
					...
					<%
						...
						bestEpochs = FFANNAdaptiveBackPropagationJSP.getBestEpochs();
					%>
				...
				<table border=0 width=100%>
					<%
						...
						int epochsSHL = 0;
						for (int i = 0; i < FFANNAdaptiveBackPropagationJSP
								.getQualitySHLArray().length; i++) {
							qualitySHL = FFANNAdaptiveBackPropagationJSP
									.getQualitySHLArray(i);
							epochsSHL = FFANNAdaptiveBackPropagationJSP.getEpochsSL(i);
					%>
					<tr>
				...
				<table border=0 width=100%>
					<%
						...
						for (int i = 0; i < FFANNAdaptiveBackPropagationJSP
							...
							qualityMHL = FFANNAdaptiveBackPropagationJSP
									.getQualityMHLArray(i);
							epochsMHL = FFANNAdaptiveBackPropagationJSP.getEpochsML(i);
					%>
					<tr>
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				<form id="form1" name="form1" method="post"
					action="backpropAdaptiveSingleDetails.jsp">
					<table>
						<tr>
							<td>
								<div id="SHLAgentDetails">
									<label for="SHLAgentDetails">SHL agent evolution
										...
										name="showSHLAgentDetails" size="1" id="showSHLAgentDetails">
											<%
												for (int networks = 0; networks < FFANNAdaptiveBackPropagationJSP
