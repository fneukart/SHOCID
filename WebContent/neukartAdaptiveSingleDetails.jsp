...
<body>
	<h1 class="heading">Adaptive genetic Neukart classification
		results</h1>
	<input type=button value="Back" onCLick="history.back()">
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = NeukartAdaptiveJSP
					.getNumberInputNeurons();
			MLDataSet trainingSet = NeukartAdaptiveJSP
					.getTrainingSet();
			BasicNetwork network = NeukartAdaptiveJSP
					.getBestNetwork();
			int col = 0;
			double[][] inputValuesArray = NeukartAdaptiveJSP
					.getInputValues(0);
			double[][] outputValuesArray = NeukartAdaptiveJSP
					.getOutputValuesBestOverall();
			double[][] idealValuesArray = NeukartAdaptiveJSP
					.getIdealValues(0);
			double[] error = NeukartAdaptiveJSP
					.getErrorBestNetwork();
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				...
				<%
					...
					if (NeukartAdaptiveJSP.getErrorsSL(SHLAgentNumber).length > 500) {
						errorSelected = new double[500];
						jump = Math.round(NeukartAdaptiveJSP
								.getErrorsSL(SHLAgentNumber).length / 500);

						for (int i = 0; i < 500; i++) {
							errorSelected[i] = NeukartAdaptiveJSP
									.getErrorsSL(SHLAgentNumber)[position];
							position = position + jump;
						}
					} else {
						errorSelected = NeukartAdaptiveJSP
			...
			<div class="AccordionPanelContent">
				...
				<%
					...
					if (NeukartAdaptiveJSP.getErrorsML(MHLAgentNumber).length > 500) {
						errorSelected = new double[500];
						jump = Math.round(NeukartAdaptiveJSP
