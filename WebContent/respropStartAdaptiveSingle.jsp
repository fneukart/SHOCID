...
<%@ page import="shocid.ffann.training.adaptive.FFANNAdaptiveResilientPropagationJSP"%>
...
<title>Adaptive Resilient Propagation Data Processing</title>
<link href="SpryAssets/SpryAccordion.css" rel="stylesheet"
	type="text/css">
</head>
<body>
	<h1 class="heading">Adaptive resilient propagation classification
		results</h1>
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = FFANNAdaptiveResilientPropagationJSP
					.getNumberInputNeurons();
		int numberOutputNeurons = Integer.valueOf(numberOutputNeuronsString).intValue();
			MLDataSet trainingSet = FFANNAdaptiveResilientPropagationJSP
					.getTrainingSet();
			BasicNetwork network = FFANNAdaptiveResilientPropagationJSP
					.getBestNetwork();
			int col = 0;
			double[][] inputValuesArray = FFANNAdaptiveResilientPropagationJSP
					.getInputValues(0);
			double[][] outputValuesArray = FFANNAdaptiveResilientPropagationJSP
					.getOutputValuesBestOverall();
			double[][] idealValuesArray = FFANNAdaptiveResilientPropagationJSP
					.getIdealValues(0);
			double[] error = FFANNAdaptiveResilientPropagationJSP
					.getErrorBestNetwork();
			double[] errorSelected;
			int epochs = FFANNAdaptiveResilientPropagationJSP.getBestEpochs();
		%>
		<div class="AccordionPanel">
			<div class="AccordionPanelTab">Error over iterations - graph of
				resilient propagation agent</div>
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				<%
					...
					if (FFANNAdaptiveResilientPropagationJSP.getBestSHLNetworkQuality() < FFANNAdaptiveResilientPropagationJSP
							.getBestMHLNetworkQuality()) {
						bestNetwork = FFANNAdaptiveResilientPropagationJSP
				...
				<p>
					<%
						if (FFANNAdaptiveResilientPropagationJSP.getBestSHLNetworkQuality() < FFANNAdaptiveResilientPropagationJSP
								.getBestMHLNetworkQuality()) {
							hn1 = FFANNAdaptiveResilientPropagationJSP
					...
					<%
							...
							hn1 = FFANNAdaptiveResilientPropagationJSP
									.getBestHiddenLayer1Neurons();
							hn2 = FFANNAdaptiveResilientPropagationJSP
					...
					<%
						...
						bestEpochs = FFANNAdaptiveResilientPropagationJSP.getBestEpochs();
					%>
				...
				<table border=0 width=100%>
					<%
						...
						int epochsSHL = 0;
						for (int i = 0; i < FFANNAdaptiveResilientPropagationJSP
								.getQualitySHLArray().length; i++) {
							qualitySHL = FFANNAdaptiveResilientPropagationJSP
									.getQualitySHLArray(i);
							epochsSHL = FFANNAdaptiveResilientPropagationJSP.getEpochsSL(i);
					%>
					<tr>
				...
				<table border=0 width=100%>
					<%
						...
						for (int i = 0; i < FFANNAdaptiveResilientPropagationJSP
							...
							qualityMHL = FFANNAdaptiveResilientPropagationJSP
									.getQualityMHLArray(i);
							epochsMHL = FFANNAdaptiveResilientPropagationJSP.getEpochsML(i);
					%>
					<tr>
		...
		<div class="AccordionPanel">
			...
			<div class="AccordionPanelContent">
				<form id="form1" name="form1" method="post"
					action="respropAdaptiveSingleDetails.jsp">
					<table>
						<tr>
							<td>
								<div id="SHLAgentDetails">
									<label for="SHLAgentDetails">SHL agent evolution
										...
										name="showSHLAgentDetails" size="1" id="showSHLAgentDetails">
											<%
												for (int networks = 0; networks < FFANNAdaptiveResilientPropagationJSP
