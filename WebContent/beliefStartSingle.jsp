...
<%@ page import="shocid.boltzmann.FFANNDBNSA"%>
...
<title>Deep belief network processing</title>
<link href="SpryAssets/SpryAccordion.css" rel="stylesheet"
	type="text/css">
</head>
<body>
	<h1 class="heading">Deep belief network classification results</h1>
...
// 				+ request.getParameter("output");
	...
	<div id="AccordionSingle" class="Accordion">
		<%
			int numberInputNeurons = FFANNDBNSA
					.getNumberInputNeurons();
        int numberOutputNeurons = Integer
        .valueOf(numberOutputNeuronsString).intValue();
			MLDataSet trainingSet = FFANNDBNSA
					.getTrainingSet();
			BasicNetwork network = FFANNDBNSA.getNetwork();
			int col = 0;
			double[][] inputValuesArray = FFANNDBNSA
					.getInputValues();
			double[][] outputValuesArray = FFANNDBNSA
					.getOutputValues();
			double[][] idealValuesArray = FFANNDBNSA
					.getIdealValues();
			double[] error = FFANNDBNSA.getError();
			int epochs = FFANNDBNSA.getEpochs();
		%>
		<div class="AccordionPanel">
			<div class="AccordionPanelTab">Error over iterations - graph of
				Deep Belief agent</div>
