...
public class NeuralGeneticAlgorithm extends BasicTraining implements MultiThreadable {
	...
	public class NeuralGeneticAlgorithmHelper extends BasicGeneticAlgorithm {
		...
		//added by FNNT
		/**
		 * Manipulate the current best neural network by a transgenetic vector.
		 * 
		 * @return The current best neural network.
		 */
		public void setNetwork(BasicNetwork network) {
			final Genome genome = getPopulation().getBest();
			genome.setOrganism((Object) network);
		}
	...
	/**
	 * @return The network that is being manipulated by a transgenetic vector.
	 */
	//added by FNNT
	public void setNetwork(BasicNetwork network) {
		getGenetic().setNetwork(network);
	}

	//added by FNNT
	/**
	 * Perform one danger theory hyperrecombination iteration.
	 */
	public void dangerTheoryIteration(int countToMate, int offspringCount, double worstLastIteration) {
		...
		"Performing Genetic iteration.");
		preIteration();
		getGenetic().dangerTheoryIteration(countToMate, offspringCount, worstLastIteration);
		setError(getGenetic().getError());
		postIteration();
	}
