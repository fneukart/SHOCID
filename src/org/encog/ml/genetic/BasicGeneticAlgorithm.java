...
public class BasicGeneticAlgorithm extends GeneticAlgorithm {
	 ...
	 * @throws NeuralNetworkException
	 */
	public void dangerTheoryIteration(int countToMate, int offspringCount, double worstLastIteration)
	{
		...
		final int matingPopulationSize = (int) (getPopulation().getPopulationSize() * getMatingPopulation());
		...
		for (int i = 0; i < countToMate; i++) {
			...
			/*if the quality of the childs is better then the quality of the worst in the last iteration, they are being added
			 * to the population. otherwise, they are being deleted
			 * 
			 */
			if (getPopulation().getGenomes().get(offspringIndex).getScore() < (worstLastIteration/110)*100 && getPopulation().getGenomes().get(offspringIndex).getScore() < (worstLastIteration/110)*100)
			{
				offspringIndex+=2;
				//System.out.println("offsprings better than worst of the last iteration");
			}
			else
			{
				//System.out.println("offsprings worser than worst of the last iteration");
				i-=1;
			}
